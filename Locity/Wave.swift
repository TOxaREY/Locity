...
class Wave: SKView {
    ...
    @objc func waveAndShip() {
        ...
        let rotation1 = SKAction.rotate(byAngle: -0.15, duration: 0.75)
        let rotation2 = SKAction.rotate(byAngle: 0.15, duration: 0.75)
        let rotation3 = SKAction.rotate(byAngle: 0.15, duration: 0.75)
        let rotation4 = SKAction.rotate(byAngle: -0.15, duration: 0.75)
        ...
        wave.run(moveLeft, completion: {self.wave.run(moveBottom); self.wave2.run(moveBottom, completion: {self.scene!.removeAllChildren();NotificationCenter.default.post(name: NSNotification.Name(rawValue: "homeButtonEnable"), object: nil)})})
        wave2.run(moveRight)
        ship.run(moveShip)
